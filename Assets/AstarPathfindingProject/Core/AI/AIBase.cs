...
namespace Pathfinding {
	...
	public abstract class AIBase : VersionedMonoBehaviour {
		...
		void FinalizePosition (Vector3 nextPosition) {
			...
			// update animations
			Vector3 moveDir = (nextPosition - currentPosition + accumulatedMovementDelta).normalized;
            Animator animator = GetComponent<Animator>();
            animator.SetFloat("X", moveDir.x);
            animator.SetFloat("Y", moveDir.y);
			if(moveDir != Vector3.zero && animator.GetBool("IsWalking") == false)
			{
				animator.SetBool("IsWalking", true);
			}
			if(moveDir == Vector3.zero && animator.GetBool("IsWalking") == true)
			{
